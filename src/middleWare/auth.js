  ...
  try {
    ...
    if (!decoded) {
      return res
        ...
        .send({
          status: false,
          message: "Invalid authentication token in request headers ⚠️",
        });
    }
...
const authorize = async function (req, res, next) {
  try {
    ...
    if (!bookToBeModified)
      return res
        .status(400)
        .send({ status: false, message: "Book Id must be present in params" });
    if (!mongoose.isValidObjectId(bookToBeModified))
    ...
    if (!newUserId)
      return res
        .status(400)
        .send({ status: false, message: "Please use correct bookId" });
