...
const createReview = async function (req, res) {
  try {
    ...
    if (checkId.isDeleted == true)
      return res
        .status(404)
        .send({ status: false, message: "this book is deleted" });
    ...
    let { review, reviewedBy, rating ,tokenId} = data;
    ...
    if (reviewedBy == "")
      return res
        .status(400)
        .send({ status: false, message: "reviewedBy can't be empty" });
    if (reviewedBy) {
      if (!/^[a-zA-Z,\-.\s]*$/.test(reviewedBy))
        return res
          ...
          .send({
            status: false,
            message: "provide a valid name in reviewedBy field",
          });
    ...
    if (!rating)
      return res.status(400).send({ status: false, message: "enter rating" });
    if (!/^[1-5]?$/.test(rating))
      return res
        .status(400)
        .send({ status: false, message: "provide a valid rating" });
    ...
    res
      .status(201)
      .send({ status: true, message: "review created", data: showData });
  } catch (err) {
    //console.log(err);
...
const getReviews= async function (req,res){
  ...
  let data=await reviewModel.find(params)
  return res
  .status(200)
  .send({ status: true, message: "List of reviews", data: data });
}catch(err){
  return res.status(500).send({ status: false, mag: err.message });
}
}
...
const updateReview = async function (req, res) {
  try {
    ...
    if (!mongoose.isValidObjectId(reviewId))
      return res
        .status(400)
        .send({ status: false, message: "provide valid reviewId" });
    let checkreview = await reviewModel.findOne({
      _id: reviewId,
      bookId: bookId,
    });
    if (!checkreview)
      return res.status(404).send({ status: false, message: "no such review" });
    if (checkreview.isDeleted == true)
      return res
        .status(404)
        .send({ status: false, message: "this review is deleted" });
    ...
    if (rating == "")
      return res
        .status(400)
        .send({ status: false, message: "rating can't be empty" });
    if (rating) {
      if (!/^[1-5]?$/.test(rating))
        return res
          .status(400)
    ...
    res
      ...
      .send({
        ...
        data: { ...checkId._doc, reviewsData: updateDoc },
      });
  } catch (err) {
    return res.status(500).send({ status: false, mag: err.message });
  }
...
const deleteReview = async function (req, res) {
  try {
    ...
    if (checkId.isDeleted == true)
      return res
        .status(404)
        .send({ status: false, message: "this book already is deleted" });
