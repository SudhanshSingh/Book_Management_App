...
const createUser = async function (req, res) {
  try {
    ...
    if (!title)
      return res
        .status(400)
        .send({ status: false, message: "Please include a title" });
    let arr = ["Mr", "Mrs", "Miss"];
    let titleCheck = arr.includes(title);
    if (!titleCheck)
      return res
        .status(400)
        .send({ status: false, message: "Enter a valid title-Mr,Mrs,Miss" });

    if (!name)
      return res
        .status(400)
        .send({ status: false, message: "Please include the name" });
    ...
    if (!isValid(phone)) {
      return res
        .status(400)
        .send({ status: false, message: "Phone number should be in string."});
    }
    if (phone.includes(" ")) {
      {
        return res
    ...
    if (!email) {
      return res
        .status(400)
        .send({ status: false, message: "Please include an email" });
    }
    const vaildEmail = /^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/.test(
      email
    );
    if (!vaildEmail)
      return res
        ...
        .send({
          status: false,
          message: "Email is invalid.Please use correct EmailId",
        });
    if (email.includes(" ")) {
      {
        return res
          ...
          .send({
            status: false,
            message: "Please remove any empty spaces in email",
          });
      }
    }
    ...
    if (!password) {
      return res
        .status(400)
        .send({ status: false, message: "Please include a password" });
    }
    if (!isValid(password)) {
      return res
        .status(400)
    ...
    if (userData.address) {
      ...
      if (userData.address.street == "")
        return res
          .status(400)
          .send({ status: false, message: "street can't be empty" });
      if (userData.address.street) {
        if (!isValid(userData.address.street)) {
          return res
            .status(400)
            .send({
              status: false,
              message: "Street address is not valid address",
            });
        }
      }
      ...
      if (userData.address.city == "")
        return res
          .status(400)
          .send({ status: false, message: "city can't be empty" });
      if (userData.address.city) {
        if (!/^[a-zA-Z]+(?:[\s-][a-zA-Z]+)*$/.test(userData.address.city))
          return res
            .status(400)
            .send({ status: false, message: "provide a valid city" });
        if (!isValid(userData.address.city)) {
          return res
            .status(400)
            .send({
              status: false,
              message: "City address is not valid address",
            });
        }
      }
      ...
      if (userData.address.pincode == "")
        return res
          .status(400)
          .send({ status: false, message: "pincode can't be empty" });
      if (userData.address.pincode) {
        if (!/^[1-9][0-9]{5}$/.test(userData.address.pincode))
          return res
            .status(400)
            .send({ status: false, message: "provide a valid pincode." });
        if (!isValid(userData.address.pincode))
          return res
            .status(400)
            .send({
              status: false,
              message: "Address pincode  is not valid pincode.",
            });
        if (userData.address.pincode.includes(" "))
          return res
            .status(400)
            .send({
              status: false,
              message: "Please remove any empty spaces from Address pincode",
            });
      }
    }
    ...
    return res
      ...
      .send({
        status: true,
        message: "UserData is successfully created",
        data: savedData,
      });
  } catch (err) {
...
async function loginUser(req, res) {
  try {
    ...
    if (!email)
      return res
        .status(400)
        .send({ status: false, message: "Please include a email" });
    ...
    if (!findUser)
      return res
        ...
        .send({
          status: false,
          message: "Invalid credentials. Please check the details & try again.",
        });
